...
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def visualize_top_obs(self, env_runner, top_obs, top_obs_dicts, save_tag_folder):
        ...
        for obs_idx in range(len(top_obs)):
            ...
            human_label = input("Enter human label (avoid, recover): ")
            if human_label == 'avoid':
                obs_to_label['avoid'].append(top_obs[obs_idx])
            elif human_label == 'recover':
                obs_to_label['recover'].append(top_obs[obs_idx])
        ...
        with JsonLogger(log_path) as json_logger:
            ...
            for active_learning_round in range(num_active_learning_rounds):
                ...
                if active_learning_round > 0:
                    ...
                    # visualize top 10
                    human_labels_ = self.visualize_top_obs(env_runner, top_obs, top_obs_dicts, save_tag_folder)
                    # add to human_labels
                    for key in human_labels_:
                        if key not in human_labels:
                            human_labels[key] = []
                        human_labels[key].extend(human_labels_[key])
                ...
                for local_epoch_idx in range(training_active_interval):
                    ...
                    with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(tepoch):
                            ...
                            # compute loss
                            raw_loss = self.model.compute_loss(batch, human_labels)
                            loss = raw_loss / cfg.training.gradient_accumulate_every
                            loss.backward()
