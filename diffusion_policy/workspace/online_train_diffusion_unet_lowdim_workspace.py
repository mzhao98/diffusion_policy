...
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def train_inv_dyn_model(self, model, dataset, recovery_dataset, cfg, active_learning_round, device, n_epochs_inv_dyn):
        ...
        # evaluate model and plot 2d predictions
        total_error = 0
        counter = 0
        for ep in range(n_episodes):
            if ep == 0:
            ...
            for c in range(start_idx, end_idx):
                ...
                # unnormalize
                pred_y = model.normalizer['action'].unnormalize(pred_y)
                target_y = model.normalizer['action'].unnormalize(target_y)

                # error between pred_y and target_y
                mse = torch.nn.functional.mse_loss(pred_y, target_y)
                # print("mse", mse.item())
                total_error += mse.item()

                # plot 2d predictions
                # reshape to 2d
                pred_y = pred_y.reshape(8, 2).detach().cpu().numpy()
                target_y = target_y.reshape(8, 2).detach().cpu().numpy()
                # pdb.set_trace()
                # plot
                # plt.xlim(0, 450)
                # plt.ylim(0, 450)
                # plt.scatter(pred_y[:,0], pred_y[:,1], color='r')
                # plt.scatter(target_y[:,0], target_y[:,1], color='b')
                # plt.plot(pred_y[:,0], pred_y[:,1], color='r')
                # plt.plot(target_y[:,0], target_y[:,1], color='b')
                # plt.legend(['pred', 'gt'])
                # plt.show()
        print("total_error", total_error)
        print("average error", total_error/counter)
        # pdb.set_trace()
    ...
    def run_active_learning_online(self):
        ...
        with JsonLogger(log_path) as json_logger:
            ...
            for active_learning_round in range(num_active_learning_rounds):
                ...
                runner_log, obs_to_diff, human_label_to_obs = env_runner.run(policy, self.inv_model)
