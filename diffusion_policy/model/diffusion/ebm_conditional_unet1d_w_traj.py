...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self, 
        ...
        # take gradient of the output with respect to the input
        # x.register_hook(lambda grad: print(grad))

        # take gradient
        # Compute gradient of the output w.r.t. input sample
        # loss = input_x.mean()  # Can use any loss; mean simplifies the gradient calculation
        # loss.backward()  # Compute gradients

        # Extract gradient w.r.t. input
        # grad_sample = sample.grad

        x = 0.5 * (x ** 2)
        x = x.sum()
        # set output x requires grad to be True
        # x.requires_grad_(True)

        eps = torch.autograd.grad([x], [sample], create_graph=True)[0]
        # print("eps:", eps)
        ...
        #
        # pdb.set_trace()
        x = einops.rearrange(eps, 'b t h -> b h t')
