...
def plot_interactive(imgs_over_episode, uncertainty_over_episode):
    ...
    def run(self, policy: BaseLowdimPolicy, inv_model):
        ...
        for rollout_ep in range(n_envs):
            ...
            while not done:
                ...
                if action_mode == 'robot':
                    # predict model
                    pred_y = inv_model(input_x_flat)

                    # unnormalize pred_y and target_y
                    pred_y = policy.normalizer['action'].unnormalize(pred_y)
                    target_y = policy.normalizer['action'].unnormalize(target_y)

                    diff = np.linalg.norm(pred_y.cpu().detach().numpy() - target_y.cpu().detach().numpy())
                ...
                if diff > 150 and queried_uncertainty is False and timestep > 1:
        ...
        # save label_to_obs
        # with open(pathlib.Path(self.savetag_folder) / 'label_to_obs.pkl', 'wb') as f:
        #     pickle.dump(label_to_obs, f)
        ...
        return log_data, obs_to_diff, label_to_obs
