    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                # add random conditioning to obs
                # conditioning should be 1d
                # pdb.set_trace()

                # reshape obs to be 4d with shape (batch, n_obs_steps=2, Do, -1)
                # obs = obs.reshape(obs.shape[0], self.n_obs_steps, Do, -1)


                # # conditioning = np.ones((1, self.n_obs_steps))
                # # multiply by a uniform random number between 0 and 1
                # # conditioning *= np.random.uniform(0, 1, conditioning.shape)
                # # append conditioning to obs at 3rd dim
                # # obs_test = np.concatenate([obs, conditioning], axis=-1)
                # obs_1 = obs[:,:,:,0]

                # # add a random vector to obs_1 at the third dim
                # # random_vector = np.random.uniform(0, 1, (obs_1.shape[0], obs_1.shape[1], 1))
                # random_vector = np.ones((obs_1.shape[0], obs_1.shape[1], 1))
                # # concatenate random_vector to obs_1 at the third dim
                # obs_1 = np.concatenate([obs_1, random_vector], axis=-1)
                
                # obs_2 = obs[:,:,:,1]
                # # add a random vector to obs_2 at the third dim
                # # random_vector = np.random.uniform(0, 1, (obs_2.shape[0], obs_2.shape[1], 1))
                # random_vector = np.zeros((obs_2.shape[0], obs_2.shape[1], 1))
                # # concatenate random_vector to obs_2 at the third dim
                # obs_2 = np.concatenate([obs_2, random_vector], axis=-1)

                # # concatenate obs_1 and obs_2 at the fourth dim
                ...
                #     plt.plot(points_x, points_y, color=np.random.rand(3,))
                # plt.show()
