...
class Push2dKeypointsDistributionVisualizerRunner(BaseLowdimRunner):
    def __init__(self,
            output_dir,
    ...
    def render_obs_with_samples(self, env, obs, action, list_of_sampled_actions, iter_idx):
        block_position = env.envs[0].env.env.block.position
        block_x = block_position[0]
        block_y = block_position[1]
        agent_position = env.envs[0].env.env.agent.position
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                subtimestep_to_kde = self.render_obs_with_samples(env, obs, action, list_of_sampled_actions, iter_idx)
        ...
        # import pdb; pdb.set_trace()
        # saved the kde data
        with open('observation_to_kde.pkl', 'wb') as f:
            pickle.dump(observation_to_kde, f)
        print("saved observation_to_kde.pkl")
