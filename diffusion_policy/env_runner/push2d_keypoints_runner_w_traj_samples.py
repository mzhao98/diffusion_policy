...
class Push2dKeypointsDistributionTrajRunner(BaseLowdimRunner):
    def __init__(self,
            output_dir,
    ...
    def render_obs_with_samples(self, env, obs, action, list_of_sampled_actions, traj_idx, iter_idx, context_idx, list_of_sampled_likelihoods):
        ...
        plt.savefig(f'distribution_viz/d{traj_idx}_context_{context_idx}_gameboard_{iter_idx}.png')
        plt.close()
        ...
        plt.savefig(f'distribution_viz/d{traj_idx}_context_{context_idx}_variance_{iter_idx}.png')
        plt.close()
    ...
    def run_optimize_context(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            for buffer_idx in range(self.replay_buffer.n_episodes):
                # buffer_idx = 0
                # start rollout
                ...
                for t in range(buffer_idx_start, buffer_idx_end, 4):
                    ...
                    n_trajs = self.replay_buffer.n_episodes
                    ...
                    with open(f'distribution_viz/gt_idx_{buffer_idx}_timestep_to_divergences_{iter_idx}.pkl', 'wb') as f:
    ...
    def run_with_target(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            utterance = input("Enter the context: ")
            self.get_utterance_embedding(utterance)
            ...
            for t in range(buffer_idx_start, buffer_idx_end, 8):
                ...
                while continue_to_next_step == False:
                    utterance = input("Enter the context: ")
                    if utterance == 'c':
                    ...
                    if utterance == 'none':
                        ...
                        index_conditioning = np.ones((obs[...,:self.n_obs_steps,:Do].shape[0], 1), dtype=np.float32) * float(utterance)
                        # add additional dim to index_conditioning
                        index_conditioning = np.expand_dims(index_conditioning, axis=1)
                        print("index_conditioning", index_conditioning.shape)
                        # create obs dict
                    ...
                    #                                                 list_of_sampled_likelihoods)
                    subtimestep_to_kde = self.render_obs_with_samples(env, obs, gt_action, 
                                                                    list_of_sampled_actions, buffer_idx, iter_idx, utterance, 
                                                                    list_of_sampled_likelihoods)
